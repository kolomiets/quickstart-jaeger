...
|*14269*| HTTP | [*in-memory* storage backend]: 
health check at `/` and Prometheus metrics at `/metrics` for both 
...
|*16687*| HTTP |  [only for *elasticsearch* storage backend]: 
health check at `/` and Prometheus metrics at `/metrics` for
